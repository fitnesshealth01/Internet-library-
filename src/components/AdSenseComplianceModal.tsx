...
import { X, Shield, FileText, Info, Mail, Check, AlertCircle } from 'lucide-react';
...
export default function AdSenseComplianceModal({ isOpen, onClose, initialTab = 'privacy' }: AdSenseComplianceModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div 
        id="adsense-compliance-container"
        className="relative w-full max-w-4xl h-[85vh] flex flex-col bg-[#121214] border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden"
      >
        {/* Modal Header */}
        ...
        {/* Tab Selection Row */}
        <div className="flex border-b border-zinc-800 bg-zinc-950/20 overflow-x-auto scrollbar-none">
          {[
            { id: 'about', label: 'About & E-E-A-T', icon: Info },
            { id: 'privacy', label: 'Privacy Policy', icon: Shield },
            { id: 'terms', label: 'Terms of Service', icon: FileText },
            { id: 'contact', label: 'Editorial Contact', icon: Mail },
          ].map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-3.5 text-xs font-bold uppercase tracking-wider border-b-2 transition-all cursor-pointer whitespace-nowrap ${
                  isActive
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 text-zinc-300 font-sans leading-relaxed text-sm">
          
          {/* TAB 1: ABOUT & EEAT (Crucial for Google News and Discover validation!) */}
          {activeTab === 'about' && (
            <div className="space-y-6">
              ...
              <div className="space-y-3 pt-2">
                <h2 className="text-base font-bold text-white">Trust & Authenticity Standards</h2>
              ...
              <div className="p-4 bg-indigo-950/15 border border-indigo-550/20 rounded-xl text-xs text-indigo-300">
          ...
          {/* TAB 2: PRIVACY POLICY (Crucial for AdSense approval!) */}
          {activeTab === 'privacy' && (
            <div className="space-y-6">
              <div>
                <h1 className="text-2xl font-black text-white tracking-tight mb-2">
          ...
          {/* TAB 4: CONTACT PORTAL (Highly prioritized for Google Publisher verification!) */}
          {activeTab === 'contact' && (
            <div className="space-y-6">
              <div>
                <h1 className="text-2xl font-black text-white tracking-tight mb-2">
              ...
              {formSubmitted ? (
                <div className="p-6 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-center space-y-3 animate-scale-up">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 px-6 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:bg-zinc-800 text-white font-bold text-xs uppercase tracking-widest cursor-pointer shadow-md hover:shadow-lg transition-all"
                  >
                    {isSubmitting ? 'Transmitting Telegram...' : 'Send Message to Desk'}
                  </button>
                </form>
              )}

              <div className="pt-4 border-t border-zinc-800/80 space-y-2 text-xs text-zinc-500">
        ...
        {/* Modal Footer AdSense Reminder Bar */}
        <div className="px-6 py-3 border-t border-zinc-800 bg-zinc-950/40 text-[10px] text-zinc-500 text-center flex flex-col md:flex-row items-center justify-between gap-2">
          <span>This publication center complies strictly with Google Webmaster and AdSense Publisher agreements.</span>
          <span className="font-mono font-bold text-indigo-400">SECURE DISPATCH ENCRYPTED</span>
        </div>
      </div>
    </div>
  );
